...
ORIGINAL_LABELS = "/workdir/data/meta/train_labels.csv"


def run(args):
    df = pd.read_csv(ORIGINAL_LABELS)
    df = df.loc[df.distance == args.distance]
    folder = os.path.join(args.output, f"d{str(args.distance)}")
    ...
    for _, row in tqdm(df.iterrows()):
        ...
        try:
            image = cv2.imread(os.path.join(ORIGINAL_DATA_TRAIN, fname))
            image = cv2.putText(
                img=image,
